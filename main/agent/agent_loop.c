        ...
        while (iteration < MIMI_AGENT_MAX_TOOL_ITER) {
            ...
            if (!resp.tool_use) {
                ...
                if (iteration == 0 && is_audio_request(msg.content)) {
                    ...
                    bool no_sound_feedback =
                        strstr(msg.content, "\xE6\xB2\xA1\xE6\x9C\x89\xE5\xA3\xB0\xE9\x9F\xB3") || /* 没有声音 */
                        strstr(msg.content, "\xE5\x90\xAC\xE4\xB8\x8D\xE5\x88\xB0") ||             /* 听不到 */
                        contains_ci(msg.content, "no sound");
                    ...
                    if (no_sound_feedback) {
                        tool_registry_execute("audio_test_tone",
                                              "{\"freq\":880,\"duration_ms\":3000}",
                    ...
                    snprintf(forced_msg, sizeof(forced_msg),
                             "Executed real audio tool calls (LLM returned no tool call).\n- audio_volume: %s\n- audio action: %s",
                             out_vol, out_act);
